...
PROMPT='$(build_prompt)'
RPROMPT='%(?.%{${fg[green]}%}✓%{${reset_color}%}.%{${fg[red]}%}✘%{${reset_color}%}) %{$reset_color%}$ff_clock_symbol %T %{$fg_bold[white]%}$ff_date_symbol %D%{$reset_color%}'
...
function enrich_append {
  ...
  function custom_build_prompt {
    ...
    if [[ $is_a_git_repo == true ]]; then
      ...
      CURRPATH=`pwd | sed 's/^\///'`
      SEP_PATH="${CURRPATH//\// $ff_separator_symbol }"
      prompt="${black_on_white} "
